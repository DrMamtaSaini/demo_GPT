    ...
    # Section 3: Student Assessment Assistant
        ...
        if st.button("Generate and Store Reports with Personalized Material"):
            if student_id and assessment_id and email_id and question_paper and marking_scheme and answer_sheet:
                ...
                Provide a detailed assessment report with:
                1. Question Analysis
                2. Summary Report
                """
                response = openai.ChatCompletion.create(
                ...
                attachments = [assessment_report_file, learning_material_file, assignment_file]

                email_body = f"""
                Dear Parent/Guardian,

                Attached are the assessment report, personalized learning material, and an assignment for your child. This will help them improve in identified areas.

                Best regards,
                Your School Name
                """
                send_email_with_attachments(email_id, "Assessment Report and Learning Material", email_body, attachments)
                st.success(f"Assessment Report and personalized materials have been sent to {email_id}.")
            else:
