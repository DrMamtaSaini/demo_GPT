...
def initialize_firebase():
    try:
        if not firebase_admin._apps:  # Avoid multiple Firebase initializations
            service_account_path = st.secrets["firebase"]["service_account_path"]
            cred = credentials.Certificate(service_account_path)
            initialize_app(cred)
        return firestore.client()
    except FileNotFoundError:
        st.error("Service account file not found. Check the path in your secrets.")
        st.stop()
...
def get_paypal_access_token():
    try:
        ...
        st.write("Access Token Response:", token_data)  # Debugging log
...
def create_paypal_order(access_token, amount, description):
    try:
        ...
        order_data = {
            ...
            "application_context": {
                "return_url": "https://f018-2c0f-fe98-9039-0-35a7-859b-867-5837.ngrok-free.app/?page=success",
                "cancel_url": "https://f018-2c0f-fe98-9039-0-35a7-859b-867-5837.ngrok-free.app/?page=cancel",
            },
        }
        ...
        st.write("Order Creation Response:", order_response)  # Debugging log
...
def success_page():
    ...
    # Retrieve query parameters from the redirect URL
    query_params = st.experimental_get_query_params()
    order_id = query_params.get("token", [""])[0]
    payer_id = query_params.get("PayerID", [""])[0]

    if not order_id or not payer_id:
        st.error("Order ID or Payer ID missing in the redirect URL.")
        return

    # Generate PayPal access token
    access_token = get_paypal_access_token()
    if not access_token:
        st.error("Failed to retrieve PayPal access token.")
        return

    # Capture the PayPal order
    capture_response = capture_paypal_order(access_token, order_id)
    if not capture_response:
        st.error("Failed to capture PayPal order.")
        return

    # Extract payment details from the response
    status = capture_response.get("status")
    transaction_id = capture_response.get("id")
    payer_email = capture_response.get("payer", {}).get("email_address", "N/A")
    amount = float(capture_response.get("purchase_units", [])[0].get("amount", {}).get("value", 0.0))
    currency = capture_response.get("purchase_units", [])[0].get("amount", {}).get("currency_code", "USD")

    if status == "COMPLETED":
        st.success("Payment completed successfully!")
        st.write("Transaction Details:", capture_response)

        # Save transaction details to Firestore
        school_id = st.session_state.get("school_id", "unknown_school")
        payment_plan = st.session_state.get("payment_description", "Unknown Plan")
        ...
        try:
            db.collection("transactions").add({
                ...
                "amount": amount,
                "status": status,
                "timestamp": firestore.SERVER_TIMESTAMP,  # Correct timestamp
                "school_id": school_id,
                "payment_plan": payment_plan,
                "payer_email": payer_email,
                "currency": currency,
                "transaction_id": transaction_id,
                "payment_method": "PayPal"  # Specify as needed
            })
            st.success("Transaction saved to the database.")
        except Exception as e:
            st.error(f"Error saving transaction to Firestore: {e}")
    else:
        st.error(f"Payment not completed. Status: {status}")
...
def capture_paypal_order(access_token, order_id):
    ...
    try:
        ...
        st.write("Capture Response:", capture_response)  # Debugging
        return capture_response
    except requests.exceptions.RequestException as e:
        st.error(f"Error capturing PayPal order: {e}")
        st.write("Response Text:", response.text if response else "No response.")
        return None
...
def cancel_page():
    ...
    if st.button("Go Back to Subscription Page"):
        st.session_state.page = "subscription_page"
...
def payment_page():
    ...
    st.title("Payment Page")
    st.write(f"Processing your payment for {description} (${amount}/month).")
